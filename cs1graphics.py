...
This is Version 1.2a1 alpha release (26 August 2011)
"""
...
_enableEvents = False       # if True, this allows for true multithreading
...
def configureMultiThreaded():
    """Configures cs1graphics to run in true multi-threaded mode.

    By default, the library is actually single-threaded, with the
    event-handling activated only when the end of the main sequence of
    commands is reached, or when an explicit wait() call is invoked on
    some graphical object.

    For most uses, our single-threaded model should suffice.  However,
    it is possible to use a true multi-threaded model, in which case
    all of the underlying rendering is managed by a secondary thread,
    and in which case event handling is activated immediately upon
    registering an event handler, even if some other sequence of
    events is being executed in the primary thread.

    Note: Some platforms, most notably certain OSX releases, will not
    properly support a multithreaded program in which the underlying Tk
    rendering is managed from a secondary thread.
    ...
    if _graphicsManager._running:
        raise GraphicsError('configuration must occur prior to other usage of the library')
...
def configureSetRecursionLimit(limit):
    ...
    to avoid an infinite recursion.  By default, that limit is 20.
...
class _Transformation(object):
    ...
    def scaleAndTranslate(self):
        return (abs(self._matrix[0] - self._matrix[3]) <= _Transformation.EPSILON and
...
class Color(object):
    ...
    def randomColor():
        ...
        return Color( (_randint(0, 255), _randint(0, 255), _randint(0, 255)) )
...
class Drawable(_EventTrigger):
    ...
    def _contentsChanged(self):
        """This should be called, for a user-defined subclass, if any
        action has taken place that would suggest a semantic change in
        the ordering or the composition of this object.

        In effect, this re-triggers the _draw sequence.
        """
        if self in _graphicsManager._frontHierarchy:
            _graphicsManager.beginRefresh()
            _graphicsManager.addCommandToQueue(('redraw', self))
            _graphicsManager.completeRefresh()
...
class Shape(Drawable):
    ...
    def setBorderDash(self, dashLength, gapLength=None):
        ...
        downLength  the length of a dash (relative to width)
        gapLength   the length of interdash space (relative to width)
                    By default, space length equals downLength

        For example, with a width of 1 the call
          setDash(4,1) gives pattern:  xxxx xxxx xxxx

          setDash(1,4) gives pattern:  x    x    x    

        With a width of 2 the call
          setDash(4,1) gives pattern:  xxxxxxxx  xxxxxxxx  xxxxxxxx
                                       xxxxxxxx  xxxxxxxx  xxxxxxxx

          setDash(1,4) gives pattern:  xx        xx        xx
                                       xx        xx        xx
...
class Canvas(_GraphicsContainer, _EventTrigger):
    ...
    def _getProperties(self):
        return { 'width': self._width, 'height': self._height, 'background color': Color(self._backgroundColor),
                 'title': self._title, 'transformation': self._transform, 'depth': (0,0),
                 'frozen' : self._frozen }
...
class _RenderedCanvas(object):
    def __init__(self, chain, properties):
        ...
        self._canvas = _Tkinter.Canvas(self._tkWin, width=self._w, height=self._h,
                                       ...
                                       background=Color.getTkColor(properties['background color']))
    ...
    def update(self, properties):
        ...
        if 'background color' in properties:
            self._canvas.config(background=Color.getTkColor(properties['background color']))
    ...
    def saveToFile(self, filename, bgcolor):
        ...
        fakeBG = self._canvas.create_polygon((0,0), (self._w,0), (self._w,self._h), (0,self._h),
                                             fill=Color.getTkColor(bgcolor),outline='')
        ...
        if 'border width' in properties or 'transformation' in properties:
            ...
            if w != self._width:
                if w == 0:              # changing from nonzero to zero
                    ...
                    configs[colorProp] = Color.getTkColor(properties.get('border color',self._borderColor))
                self._width = w
            ...
            if w != 0:     # recompute transformed width
                ...
                if self._dash is not None and self._dash[1] != 0:
                    a = min(255, max(1, int(round(self._dash[0]*self._transWidth))))
                    b = min(255, max(1, int(round(self._dash[1]*self._transWidth))))
                    # for some reason, (a,b,a,b) tuple works better than (a,b) tuple for Tkinter
                    configs['dash'] = (a,b,a,b)

        if 'border color' in properties:
            self._borderColor = properties['border color']
            c = Color.getTkColor(self._borderColor)
...
class _RenderedFillableShape(_RenderedShape):
    ...
    def update(self, properties):
        if 'fill color' in properties:
            self._canvas._canvas.itemconfigure(self._object, fill=Color.getTkColor(properties['fill color']))
...
class _RenderedPath(_RenderedShape):
    def __init__(self, chain, properties):
        ...
        if 'arrows' in properties:
            ...
            configs['arrowshape'] = '%f %f %f'%(w*8,w*10,w*3)
        ...
        if 'transformation' in properties or 'points' in properties:
            ...
            if len(self._points) > 1:
                ...
                if wasEmpty:   # need to explicitly (re)set border properties
                    configs['width'] = self._transWidth
                    configs['fill'] = Color.getTkColor(self._borderColor)
            else:
                tkCoords = 6 * (0,)
    ...
    def update(self, properties):
        ...
        if 'transformation' in properties or 'points' in properties:
            ...
            if len(self._points) > 1:
                ...
                if wasEmpty:   # need to explicitly (re)set border properties
                    configs['width'] = self._transWidth
                    configs['outline'] = Color.getTkColor(self._borderColor)
                    configs['fill'] = Color.getTkColor(self._fillColor)
            else:
                tkCoords = 6 * (0,)
            self._canvas._canvas.coords(self._object,tuple(tkCoords))
...
class _RenderedText(_RenderedDrawable):
    ...
    def __init__(self, chain, properties):
        ...
        if not transform.scaleAndTranslate() and parentTransform.scale() > 0:
        ...
        actualSize = int(round(parentTransform.scale()*properties['font size'] * _RenderedText.normalFactor))
        self._object = self._canvas._canvas.create_text(center.get(), text=properties['message'],
                                                        anchor='center', justify=properties['justify'],
                                                        fill=Color.getTkColor(properties['font color']),
...
#                raise GraphicsError('text cannot be reflected unless Python Image Library is installed', True)
        ...
        if 'font color' in properties:
            self._canvas._canvas.itemconfigure(self._object, fill=Color.getTkColor(properties['font color']))
